...
class CallbackChatManager(ChatManagerBase):
    ...
    def submit_prompt(self, prompt):
        ...
        if len(self.prompts) > 1:
            tmp_chat = []
            self._add_msg(tmp_chat, ChatRole.SYSTEM, 'The user gave their feedback on the outputs generated by the old prompt. '
            ...
            self._add_msg(tmp_chat, ChatRole.SYSTEM, 'Old prompt outputs discussion:')
            tmp_chat += prev_discussion_cot[2:-1]
            self._add_msg(tmp_chat, ChatRole.SYSTEM, 'New prompt outputs:')
            tmp_chat += self.model_chat[-len(self.examples):]
            self._add_msg(tmp_chat, ChatRole.SYSTEM, 'Decide whether the new outputs are as accepted by the user earlier. Provide your conclusion for each example.')
