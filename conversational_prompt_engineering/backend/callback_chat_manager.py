...
class ModelPrompts:
    def __init__(self) -> None:
        self.task_instruction = \
            ...
            'Try to help the user, and make suggestions when you can.\n' \
        ...
        self.api = {
            ...
            'self.end_summaries_discussion()': 'call this function after all the summaries have been discussed with the user.',
        ...
        self.analyze_result_instruction = \
            'For each example show the produced summary to the user and discuss it with them, one example at a time. ' \
            'Note that the user has not seen these summaries yet, wehen presenting a summary show its full text.\n' \
            ...
            'Continue your conversation with the user by calling submit_message_to_user API until you go through all the summaries.\n' \
            'After all the summaries were accepted by the user, call end_summaries_discussion.\n' \
            'Remember to communicate only via API calls.'
...
class CallbackChatManager(ChatManagerBase):
    def __init__(self, bam_api_key, model, conv_id) -> None:
        ...
        self.summary_discussion_state = None
    ...
    def submit_prompt(self, prompt):
        ...
        for i, f in futures.items():
            ...
            self.summary_discussion_state['produced_summaries'][i] = summary
    ...
    def end_summaries_discussion(self):
        analyze_discussion_task = \
            f'You need to decide whether the summaries produced by the prompt "{self.prompts[-1]}" ' \
        ...
        txt_produced_accepted = zip(self.examples, self.summary_discussion_state['produced_summaries'], self.summaries)
        ...
        temp_chat += [msg for msg in self.user_chat[self.summary_discussion_state['user_chat_begin']:]
